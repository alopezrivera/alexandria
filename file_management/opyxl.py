    ...
    def plot_col(self, col1, col2, i, n, args_x, args_y, cmap='inferno', last=False, fig=None):
        try:
            ...
            line(self.column_values(self.headings.index(col1)),
                 ...
                 x_label=col1.split(' ')[0].capitalize() + ' [km]' if col1.split(' ')[0] == 'altitude' else col1.capitalize(),
                 y_label=col2.split(' ')[0].capitalize() + ' [km]' if col2.split(' ')[0] == 'altitude' else col2.capitalize(),
                 ...
                 title='Stratos IV flight profile: {} vs {}'.format(chain(col2.split(' ')[:-1]).capitalize(), chain(col1.split(' ')[:-1]).capitalize()),
                 label=self.s.replace('_', ' ')[:-2],
